...
const static double SRES_ERROR = 0.4;                        //角度偏差の許容値（ここはビルドしなおさなくて済むよう、rosparamで設定できるのが望ましいが）
...
int main(int argc, char **argv)
{
    ...
    ros::init(argc, argv, "pwri_reproduction_node");
    ros::NodeHandle n;
    ...
    while (ros::ok())
    {
        ...
        case MODE2://掘削(アーム軸→曲げる,バケット軸→ショベル手前に曲げる)
            joint_cmd[SWING].data = 0.0;
            joint_cmd[BOOM].data = 0;
